import Foundation

@MainActor
enum L10n {
    static var lang: String { EyesConfig.shared.language }

    private static let table: [String: [String: String]] = [
        "zh": [
            "appearance": "外观",
            "eye_size": "眼睛大小",
            "pupil_size": "瞳孔大小",
            "eye_gap": "眼距",
            "pupil_too_large": "瞳孔太大，无法移动",
            "reset": "重置",
            "general": "通用",
            "launch_at_login": "开机自启",
            "launch_at_login_hint": "需要 .app bundle 才能启用开机自启（请使用 Xcode 构建）",
            "sleep_mode": "防睡眠维持模式",
            "sleep_mode_always": "始终维持（唤醒后自动恢复）",
            "sleep_mode_no_sleep": "睡眠/锁屏不维持，重启维持",
            "sleep_mode_never": "从不维持",
            "terminal_app": "终端应用",
            "select_terminal": "选择终端",
            "terminal_path": "终端路径",
            "browse": "浏览...",
            "shortcuts": "快捷说明",
            "shortcut_left_eye": "右键点击左眼 → 在 Finder 当前路径打开终端",
            "shortcut_right_eye": "右键点击右眼 → 切换防睡眠模式（激活时高光变红）",
            "shortcut_left_click": "左键点击菜单栏图标 → 打开菜单",
            "shortcut_right_click": "全局左键按下 → 左眼眨眼，全局右键按下 → 右眼眨眼",
            "language": "语言",
            "menu_path_label": "路径: %@",
            "menu_copy_path": "复制路径",
            "menu_open_terminal": "打开终端",
            "menu_sleep_on": "防睡眠: 开启",
            "menu_sleep_off": "防睡眠: 关闭",
            "menu_settings": "设置...",
            "menu_refresh": "刷新位置",
            "menu_about": "关于",
            "menu_quit": "退出",
            "settings_title": "SwiftEyes 设置",
            "about_title": "关于 SwiftEyes",
            "about_version": "版本",
            "about_description": "轻量级 macOS 菜单栏应用，会跟随鼠标的 Googly Eyes",
            "none": "无",
        ],
        "en": [
            "appearance": "Appearance",
            "eye_size": "Eye Size",
            "pupil_size": "Pupil Size",
            "eye_gap": "Eye Gap",
            "pupil_too_large": "Pupil too large to move",
            "reset": "Reset",
            "general": "General",
            "launch_at_login": "Launch at Login",
            "launch_at_login_hint": "Requires .app bundle to enable launch at login (build with Xcode)",
            "sleep_mode": "Sleep Prevention Persistence",
            "sleep_mode_always": "Always maintain (auto-restore on wake)",
            "sleep_mode_no_sleep": "Don't maintain on sleep/lock, maintain on restart",
            "sleep_mode_never": "Never maintain",
            "terminal_app": "Terminal App",
            "select_terminal": "Select Terminal",
            "terminal_path": "Terminal Path",
            "browse": "Browse...",
            "shortcuts": "Shortcuts",
            "shortcut_left_eye": "Right-click left eye → Open terminal at Finder path",
            "shortcut_right_eye": "Right-click right eye → Toggle sleep prevention (highlight turns red when active)",
            "shortcut_left_click": "Left-click menu bar icon → Open context menu",
            "shortcut_right_click": "Global left-click → Left eye blinks, Global right-click → Right eye blinks",
            "language": "Language",
            "menu_path_label": "Path: %@",
            "menu_copy_path": "Copy Path",
            "menu_open_terminal": "Open Terminal",
            "menu_sleep_on": "Prevent Sleep: On",
            "menu_sleep_off": "Prevent Sleep: Off",
            "menu_settings": "Settings...",
            "menu_refresh": "Refresh Position",
            "menu_about": "About",
            "menu_quit": "Quit",
            "settings_title": "SwiftEyes Settings",
            "about_title": "About SwiftEyes",
            "about_version": "Version",
            "about_description": "A lightweight macOS menu bar app with googly eyes that follow your cursor",
            "none": "None",
        ]
    ]

    static func tr(_ key: String) -> String {
        table[lang]?[key] ?? table["en"]?[key] ?? key
    }

    static func tr(_ key: String, _ args: CVarArg...) -> String {
        let fmt = tr(key)
        return String(format: fmt, arguments: args)
    }
}
